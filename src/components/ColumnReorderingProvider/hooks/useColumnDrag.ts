...
export function useColumnDrag<TData>({
    ...
    const captureOverlayClassNames = (columnId: string) => {
    ...
    const handleDragStart = (event: DragStartEvent) => {
        ...
        // Auto-scroll only makes sense for center (non-pinned) columns
    ...
    const handleDragOver = (event: DragOverEvent) => {
        setTargetColumnId((event.over?.id as string) ?? null);
    };
    ...
    const handleDragEnd = (event: DragEndEvent) => {
        ...
        const droppedOnColumnId = over.id as string;
