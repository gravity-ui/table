...
```tsx
import {useTable, useColumnsAutoSize, ColumnDef} from '@gravity-ui/table';
...
function MyTable({data}) {
  ...
  const {setTableInstance, columnsWithAutoSizes} = useColumnsAutoSize({
    columns,
    options: {
      minWidth: 80,
      maxWidth: 300,
...
| `renderElementForMeasure` | `(element?: ReactNode) => JSX.Element` | No       | Provide a custom renderer for measurement if you need some custom logic for your cells (for example context providers), you can use the default renderElementForMeasure from @gravity-ui/table inside |
...
```tsx
import {renderElementForMeasure as defaultRenderElementForMeasure} from '@gravity-ui/table';
import {Provider} from 'react-redux';

function renderElementForMeasure(element?: ReactNode) {
